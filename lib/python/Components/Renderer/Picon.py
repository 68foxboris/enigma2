...
class PiconLocator:
	...
	def getPiconName(self, serviceRef):
		...
		if not pngname:  # picon by channel name
			if (sName := ServiceReference(serviceRef).getServiceName().replace('\x80', '').replace('\x86', '').replace('\x87', '')) and "SID 0x" not in sName and (utf8Name := sanitizeFilename(sName).lower()) and utf8Name != "__":  # avoid lookups on zero length service names
