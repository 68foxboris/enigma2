		...
		if BoxInfo.getItem("isRotorTuner") and value != "no":
			if value.isdigit():
				...
				if nim_text == _("undefined"):
					...
					if saved_text:
						nim_text = saved_text
				return "%s:%s" % (r"\c0000f0f0" + chr(ord("A") + int(value)), r"\c00f0f0f0" + nim_text)
			elif value == "all":
				all_text = ""
				for x in nimmanager.nim_slots:
					print(x.slot)
					...
					if nim_text != _("rotor is not used"):
						...
						all_text += "%s:%s " % (r"\c0000f0f0" + chr(ord("A") + x.slot), r"\c00f0f0f0" + nim_text)
				return all_text
			self.LastRotorPos = config.misc.lastrotorposition.value
			...
			if rotor:
				...
				if value == "tunername":
					...
					if tuner != active_tuner:
						return "%s:%s" % (r"\c0000f0f0" + chr(ord("A") + tuner), r"\c00f0f0f0" + orbpos(config.misc.lastrotorposition.value))
					return ""
