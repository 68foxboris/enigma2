...
from Components.config import ConfigIP, ConfigMacText, ConfigNumber, ConfigPassword, ConfigSelection, ConfigSubsection, ConfigText, ConfigYesNo, NoSave, ReadOnly, config, getConfigListEntry
...
class AdapterSetupConfiguration(Screen):
	...
	def ok(self):
		...
		if self["menulist"].getCurrent()[1] == "wlanstatus":
			try:
				from Plugins.SystemPlugins.WirelessLan.plugin import WlanStatus  # noqa F401
			except ImportError:
				self.session.open(MessageBox, self.missingwlanplugintxt, type=MessageBox.TYPE_INFO, timeout=10)
			else:
				if queryWirelessDevice(self.iface):
