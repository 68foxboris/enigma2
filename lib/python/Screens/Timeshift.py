...
class TimeshiftSettings(Setup):
	...
	def pathStatus(self):
		if self["config"].getCurrentIndex() == self.pathItem:
			...
			elif stat(path).st_dev in DEFAULT_INHIBIT_DEVICES and config.usage.timeshift_skipreturntolive.value is False:  # allow timeshift on flash for audio plugins and no other volume availabe
